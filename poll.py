...
ASSOCIATE_DATA_RE = re.compile(
    r"\bassociate\b.*\b(?:data scientist|data analyst|product analyst|"
    r"business analyst|quantitative analyst|analytics consultant)\b")
...
def selected_companies(cfg):
    """Return the configured company catalog narrowed to enabled_companies.

    Existing catalog rows win so their direct ATS adapters and aliases are
    preserved. An allowlisted company not yet in the catalog is still covered
    through the aggregator feeds via a synthetic ``ats: none`` row.
    """
    targets = cfg.get("enabled_companies")
    if not targets:
    ...
    for company in cfg["companies"]:
        for name in [company["name"], *company.get("aliases", [])]:
            catalog.setdefault(key(name), company)

    selected = []
    selected_ids = set()
    for target in targets:
        company = catalog.get(key(target))
        if company is None:
            company = {"name": target, "ats": "none", "slug": ""}
        identity = key(company["name"])
        # Some parent-company catalog rows carry subsidiary aliases (for
        # example Microsoft has LinkedIn/GitHub aliases). When both brands are
        # explicitly allowlisted, keep the second one as its own aggregator
        # target instead of silently collapsing it into the parent.
...
def gather(cfg, verify=False):
    ...
    with ThreadPoolExecutor(max_workers=cfg.get("max_workers", 12)) as ex:
        for row, hits in ex.map(fetch, companies):
            report.append(row)
...
def write_report(cfg, postings, report):
    ...
    companies = selected_companies(cfg)
    live = [c for c in companies if c["ats"] != "none"]
    simp = [c for c in companies if c["ats"] == "none"]

    L = []
    L.append("# What this repo is tracking\n")
    L.append("Auto-generated by `python3 poll.py --report`. Do not hand-edit.\n")
    L.append(f"- **{len(companies)} companies** watched\n")
